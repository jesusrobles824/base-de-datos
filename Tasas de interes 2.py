...
for i, columna in enumerate(Tasas.columns):
    ...
    # plt.text(adjusted_date, last_value, f'{last_value:.1f}', 
    #         color=colors[i], fontsize=15, ha='left', va='bottom', weight='bold')
plt.suptitle('Tasas de interés', fontsize=28, color='#004F95', weight='bold', x=0.1515, y=0.966)
plt.title(f'TNA (%) de las principales tasas de mercado. Período junio 2024 a {mes_nombre} 2025.', fontsize=16, color='black', alpha=0.75, pad=80, x=0.32)
plt.gca().set_xticks([])
plt.xlabel(f'Fuente: Jesús Robles en base a BCRA. En paréntesis TNA (%) al {ultimo}.', labelpad=30, fontsize=11.5, x=0.24)
